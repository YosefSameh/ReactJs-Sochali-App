...
const Posts = (props) => {
  ...
  const handelFollow = async (id) => {
    ...
    if (errorF) {
      return toast.error(
        `Follow Not Fount ,${errorF}, Please Refresh Page and Try Agin`,
        {
          position: "top-right",
          autoClose: 3500,
  ...
  const Save = async (id) => {
    ...
    if (stateSave.error) {
      return toast.error(
        `${stateSave.error}, Please Refresh Page and Try Agin`,
        {
          position: "top-right",
          autoClose: 3500,
          hideProgressBar: false,
  ...
  const [idCommentAndPost, setIdCommentAndPost] = useState({
    idComment: "",
    idPost: "",
  });
  ...
  const handelEdite = (id) => {
    ...
    axios
      .patch(
        `https://node-js-sochali-app.vercel.app/api/posts/${id}`,
        body,
        config
      )
      .then((response) => {
  ...
  const SudmitDeletePost = (id) => {
    axios
      .delete(`https://node-js-sochali-app.vercel.app/api/posts/${id}`, config)
      .then((response) => {
        if (props.url === undefined) {
          dispatch(fetchPosts({ url, config }));
        }
    ...
    <>
            ...
            >
              <div className="d-flex justify-content-between ">
                <div className="d-flex">
                  <Link to={`/profile/${ele.user_id}`}>
                    <Avatar alt="Remy Sharp" src={ele.imgProfile} />
                  </Link>
                  ...
                  {ele.user_id === idUser
                    ...
                    : index ===
                        posts.findIndex(
                          (post) => post.user_id === ele.user_id
                        ) && (
                        <div className="ms-4 d-flex align-items-start  ">
                          {followStatus[ele._id] === false ||
                          followStatus[ele._id] === undefined ? (
                            <PersonAddIcon
                              onClick={() => handelFollow(ele._id)}
                            ...
                            <h6 style={{ transition: "0.4s" }} variant="text">
                              Following
                            </h6>
                          )}
                        </div>
              ...
              >
                ...
                >
                  ...
                  <div onClick={colorFavorite} className="ms-3">
                    {color[ele._id] === false ||
                    color[ele._id] === undefined ? (
                      <FavoriteBorderIcon
                        onClick={() => colorFavorite(ele._id)}
                        style={{ color: "black", transition: ".2s" }}
                      />
                    ) : (
                      <FavoriteBorderIcon
                        onClick={() => unColorFavorite(ele._id)}
                        style={{ color: "red", transition: ".2s" }}
                      />
                    )}
                  </div>
                </div>
                <div className="text-start ms-3 mt-2">
                  <Button
                    style={{ textAlign: "center", transition: ".4s" }}
                    className="d-flex bg-info text-black justify-content-center align-items-center"
                    onClick={() => toggleComments(ele._id)}
                    variant="contained"
                  >
                ...
                <>
                  <div className="shadow mt-4 d-flex justify-content-center align-items-center rounded">
                    ...
                    >
                      ...
                      <div>
                        <Button
                          onClick={() => handelAddComment(ele._id)}
                          variant="contained"
      ...
      >
        ...
        <DialogActions>
          ...
          <Button
            color="error"
            variant="contained"
            onClick={() => SudmitDeletePost(selectedPostId)}
            autoFocus
          >
            Delete
          </Button>
      ...
      >
        ...
        <DialogActions>
          ...
          <Button
            type="submit"
            onClick={() => handelEdite(selectedPostId)}
            variant="contained"
          >
            Edit
          </Button>
